...
The authoritative WIT now lives in `crates/wasi-model/wit/model.wit`; the records, `complete` / `complete-stream` signatures, and the `tool-grants` shape are final at `augentic:model@0.1.0`. Everything below is impl work behind that final boundary.
