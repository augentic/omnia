...
> Status: Landed — lets an embedder grow (and shrink) the guest registry after startup through an explicit async registration primitive: verify → compile → pre-instantiate → serve → publish, at install time. The `runtime!` macro surface for the §4.5/§4.6 builder API (`resolver:`, `program:`, `command_guest:` deployment keys) has also landed; see [Composing a Runtime](../docs/guides/composing-a-runtime.md#deployment-keys-resolver-http_paths-http_listener-program-command_guest). One erratum against the macro-keys draft: `program:` validation admits `resolver:` plus `command_guest:` in place of a compiled-in manifest — the draft's stricter rule contradicted its own empty-set semantics (a resolver implies a dynamic deployment that may start with no static guests). A second erratum, against §4.5's naming and status mapping: the trigger hook landed as `http_paths` (`HttpPaths`, macro key `http_paths:`), not `http_fallback` — installing it also switches the HTTP trigger to table-only routing (a sole exporter never becomes a catch-all), which the draft did not anticipate. The §4.5 status mapping is confirmed, not superseded: a declined path or `EnsureError::Unresolved` (unknown tenant) stays an ordinary 404, while `ResolveFailed` or `ExportMismatch` on a routed identity is an error-logged 500 — the seam suite's `router` scenarios are the binding spec. Registration is the primitive; the lazy resolve-on-miss hook (`GuestResolver`) and the dynamic HTTP trigger fallback (§4.5) are a thin layer over it. Complements — does not replace — the static `[[guest]]` population; a deployment that never registers behaves exactly as today. Depends: the guest registry (`crates/omnia/src/registry.rs`), host-mediated dispatch (`crates/omnia/src/dispatch/`), the programmatic `Manifest` / `DeploymentBuilder` (`crates/omnia/src/deployment/`), `omnia compile` (`crates/omnia/src/options/compile.rs`). Relates: [embedded-guest](embedded-guest.md) (the opposite trade: fully static composition at build time; this design grows composition at run time).
