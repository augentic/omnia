...
> Status: Landed — lets an embedder grow (and shrink) the guest registry after startup through an explicit async registration primitive: verify → compile → pre-instantiate → serve → publish, at install time. The `runtime!` macro surface for the §4.5/§4.6 builder API (`resolver:`, `command_guest:` deployment keys) has also landed; see [Composing a Runtime](../docs/guides/composing-a-runtime.md#advanced-deployment-keys). One erratum against the macro-keys draft: the `program:` key was first relaxed (its validation admitted `resolver:` plus `command_guest:` in place of a compiled-in manifest — the draft's stricter rule contradicted its own empty-set semantics, since a resolver implies a dynamic deployment that may start with no static guests) and then removed entirely: raw argv passthrough is now the default for `mode: command` whenever the deployment is compiled in (a manifest, or `resolver:` plus `command_guest:`), with the program name defaulting to the manifest name. A second erratum, against §4.5's naming and status mapping: the trigger hook landed as `http_paths` (`HttpPaths`, macro key `http_paths:`), not `http_fallback` — installing it also switches the HTTP trigger to table-only routing (a sole exporter never becomes a catch-all), which the draft did not anticipate. The §4.5 status mapping is confirmed, not superseded: a declined path or `EnsureError::Unresolved` (unknown tenant) stays an ordinary 404, while `ResolveFailed` or `ExportMismatch` on a routed identity is an error-logged 500 — the seam suite's `router` scenarios are the binding spec. A third erratum, against §4.6: the `command_guest:` key (and the `DeploymentBuilder::command_guest` / `Runtime::with_command_guest` API) was removed. Command mode now routes to the sole static `wasi:cli/run` exporter, or to the `[[guest]]` entry marked `command = true` (`command: true` in the macro); the marked identity keeps §4.6's fail-closed semantics, but the resolver-supplied command guest — a fully dynamic deployment faulting its command guest in on first miss — is gone with the key, so a direct command always compiles its manifest in. The seam suite's `cli` scenarios are the binding spec. A fourth erratum: the entire §4.5 pull layer was removed as unused — `GuestResolver` / resolve-on-miss (`resolver:`, `DeploymentBuilder::resolver`, `Runtime::with_resolver`, `Runtime::ensure_guest`, the single-flight machinery), the `http_paths` trigger hook and its table-only routing policy, and the pre-bound `http_listener` adoption (`http_listener:`, `DeploymentBuilder::http_listener`, `Runtime::take_http_listener`). Registration is the sole primitive: `Runtime::register` / `deregister` remain the way a registry grows after boot, a registry miss is a dispatch error, the HTTP trigger always binds `HTTP_ADDR`, and registered guests are reachable via host-mediated link dispatch and `Dispatcher::invoke` but not via HTTP (static routes cannot name late guests). A terminology erratum: the host-mediated interface list this design calls the `link` union has since been renamed `dispatch` and made deployment-wide-only configuration (top-level `dispatch = [...]` in TOML, `dispatch:` in the macro, CLI `--dispatch`; the per-guest `link` key is removed) — no behavior change, the set was always a deployment-level union fixed at bootstrap, exactly as this design assumes. Complements — does not replace — the static `[[guest]]` population; a deployment that never registers behaves exactly as today. Depends: the guest registry (`crates/omnia/src/registry.rs`), host-mediated dispatch (`crates/omnia/src/dispatch/`), the programmatic `Manifest` / `DeploymentBuilder` (`crates/omnia/src/deployment/`), `omnia compile` (`crates/omnia/src/options/compile.rs`). Relates: [embedded-guest](embedded-guest.md) (the opposite trade: fully static composition at build time; this design grows composition at run time).
