...
- **`digest: "sha256:<64 hex>"`** (a string literal) — the digest the guest's bytes must hash to, checked before wasmtime sees them; a malformed pin is a compile error. Embedded bytes cannot change after the build, so a pin on a `path:` entry is a build-time check of the artifact; a pin on a `package:` entry holds the fetched bytes to the ones the invocation named. A `declared` load takes no digest of its own — the entry's pin is the one that holds.
...
**You need this when command mode should not rely on the sole-exporter default** — a deployment with several boot-loaded guests exporting `wasi:cli/run`, or whose command guest loads at first use (a `package:` entry, or any `[[guest]]` in a manifest file), since only a guest loaded at boot can be the catch-all.

Marking one guest entry `command: true` routes command mode to that guest instead of the sole-exporter catch-all; a first-use guest so marked loads when the run drives it. The mark requires `mode: command`, and at most one guest may carry it — several are rejected at compile time (and, for a TOML manifest, at manifest validation). With several exporters the mark is also the safer form: a future guest accidentally exporting `wasi:cli/run` cannot flip the routing.
