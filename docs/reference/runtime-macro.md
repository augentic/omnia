...
`main` and `run` use the generated `Backends` bundle. `Hooks`, `manifest`, and `run_with` exist so a test drives the *same* wiring the binary runs — over `omnia_test::host::Backends`, say — without a second `runtime!` declaration in the test tree. All three entry points build the deployment the same way: what the builder names is the binary's trusted input, so a pre-compiled guest — compiled in through `path:`, or planned by the embedder's own `main` — loads under `run` and `run_with` exactly as under `main` (see the [security model](../security-model.md)).
