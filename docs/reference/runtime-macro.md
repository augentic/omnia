...
The grammar's refusals are all compile-time, spanned to the offending key: an empty `locations:` list, a second `registry` entry, and a `config` on a path entry are each rejected with a pointed diagnostic; the configuration's contents are parsed when the runtime assembles, so a malformed one fails startup, not the first load. The removed `cache:` key is refused with a migration hint: a store-backed registry acquirer (`RegistryClient::cached`) is installed by hand from a custom `Wiring::extend`.
