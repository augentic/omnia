...
| --------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `RUST_LOG`      | unset (command `info`, server `warn`)                      | Log filter (e.g. `info`, `debug`, `omnia=trace`) for the whole process — the host console and every guest alike. The filter is decided once per run: its bare level is a verbosity flag (below), else this variable's, else the mode's default — `info` for a command, `warn` for a server — and this variable's targeted directives (`tower=off`, `omnia_core=trace`) always apply on top, so `RUST_LOG=my_sdk=debug omnia -v` runs at `debug,my_sdk=debug`. Unset, a bare server run shows host warnings (a mount that failed to preopen) and nothing else; a bare command run shows the guests' progress too. The server-mode `omnia ready` readiness line is at `info`; the rest of the runtime plumbing (initializing, command-mode ready, guest lifecycle, `wasi:cli/run` bracketing) is at `debug`. Noisy dependencies (`hyper`, `h2`, `tower`, `tonic`, `opentelemetry`, `opentelemetry_sdk`, `omnia_wasi_otel`) are always muted, and a failed telemetry flush reports at `debug`. Every guest's WASI environment carries the decided filter as its `RUST_LOG`, and the process environment itself is never written. See [Verbosity flags](#verbosity-flags). |
| `OTEL_EXPORTER_OTLP_ENDPOINT` | unset (no export)                            | OTLP gRPC endpoint for exporting host traces and metrics — OpenTelemetry's own variable, with its per-signal `OTEL_EXPORTER_OTLP_{TRACES,METRICS}_ENDPOINT` taking precedence for their signal. A signal with no non-empty endpoint has no exporter, so a run with no collector configured exports nothing — guest spans and metrics included — instead of retrying against `localhost:4317` and stalling its exit on the flush. The choice is reported once at `debug`. Export errors never reach the console — the filter always mutes `opentelemetry` / `opentelemetry_sdk` / `tonic` / `tower`. |
...
`-v`/`--verbose` and `-q`/`--quiet` select the process tracing level relative to the mode's default: each `-v` is one step up the scale `off`, `error`, `warn`, `info`, `debug`, `trace`, each `-q` one step down, clamped at the ends. The flags repeat (`-vv`, `-qq`); `-v` beside `-q` is a usage error. A flag composes with whatever `RUST_LOG` the process carries, for the host console and every guest: it displaces the variable's bare level (`info`, `debug`, …) and leaves its targeted directives (`tower=off`, `my_sdk=debug`) in force, so `RUST_LOG=my_sdk=debug omnia -v` runs at `debug,my_sdk=debug` and `RUST_LOG=info omnia -v` at `debug`. No flag keeps the process variable as it stands and falls back to the mode's default only when the variable is unset (a variable that names only targets sets no bare level, as `EnvFilter` reads it), so any filter stays reachable as `RUST_LOG=<filter>` on a bare run. A token that is neither a level nor a directive is reported on stderr and dropped.
