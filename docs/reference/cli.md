...
A runtime binary generated by `omnia::runtime!` accepts the `run` subcommand — unless the invocation sets the [`program:` key](../guides/composing-a-runtime.md#deployment-keys-resolver-http_paths-http_listener-program-command_guest), which removes the host CLI entirely: such a binary has no `run` subcommand and no `--config`/`OMNIA_CONFIG` override, and forwards its argv to the guest verbatim (`mybin greet Ada`, not `mybin run -- greet Ada`) — except the reserved host log flags `--debug` / `--quiet`, which are peeled anywhere in argv and select the [host log preset](configuration.md#host-log-flags-program-binaries). Everything below describes the standard grammar. The `compile` subcommand is part of the library CLI (with the default `jit` feature) but requires a hand-written `main` to expose — the generated `main` handles `run` only.
