...
| `loader` | no     | The guest loader (`omnia:plugins/loader`): assembly links its host and installs its grant — the manifest's `on_demand` guests as the table a `declared` load may name, its read-only mounts as the roots a `path` load reads through, and its `registries` as the routing a package is fetched by. A deployment declaring `registries` requires it. |
