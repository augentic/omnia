...
```bash
<runtime> run [WASM] [--config <omnia.toml>] [--mount <spec>]... [--dispatch <interface>]... [-- <guest args>...]
```
...
| `--dispatch <interface>` | Host-mediated interface to dispatch on a guest's behalf (repeatable). Unioned with the manifest's top-level `dispatch` list.                              |
