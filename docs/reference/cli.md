...
| `loader` | no     | The guest loader (`omnia:plugins/loader`) and the registry client a `source.package` guest is fetched with: assembly links the loader host and installs its grant — the deployment's guests as the names a `declared` load may name, its read-only mounts as the roots a `path` load reads through, and its `registries` as the routing a package is fetched by. A deployment declaring `registries` or a package guest requires it. |
