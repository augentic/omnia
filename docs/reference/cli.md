...
```bash
<runtime> run [WASM] [--config <omnia.toml>] [--mount <spec>]... [--plugins <interface>]... [-- <guest args>...]
```
...
| `--plugins <interface>` | Host-mediated interface to dispatch on a guest's behalf (repeatable). Unioned with the manifest's top-level `plugins` list.                              |
