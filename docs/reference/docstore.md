...
These types and the trait live in `omnia-guest`. They are platform-agnostic -- no `#[cfg(target_arch)]` guards, no dependency on `omnia-wasi-docstore` guest bindings. Domain crates import these and use them in handler bounds.

### Types

```rust
// omnia-guest/src/document_store.rs
...
Follows the same pattern as `TableStore`, `StateStore`, etc. in `capabilities.rs`. The `#[cfg(target_arch = "wasm32")]` default impls delegate to the guest module of `omnia-wasi-docstore` (see section 3).
...
pub trait DocumentStore: Send + Sync {
    ...
    fn get(
        ...
        async move {
            omnia_wasi_docstore::store::get(store, id).await
        }
    }
    ...
    fn insert(&self, store: &str, doc: &Document) -> impl Future<Output = Result<()>> + Send {
        async move {
            omnia_wasi_docstore::store::insert(store, doc).await
        }
    }
    ...
    fn put(&self, store: &str, doc: &Document) -> impl Future<Output = Result<()>> + Send {
        async move {
            omnia_wasi_docstore::store::put(store, doc).await
        }
    }
    ...
    fn delete(&self, store: &str, id: &str) -> impl Future<Output = Result<bool>> + Send {
        async move {
            omnia_wasi_docstore::store::delete(store, id).await
        }
    }
    ...
    fn query(
        ...
        async move {
            omnia_wasi_docstore::store::query(store, options).await
        }
    }
}
```
...
use omnia_guest::document_store::{Filter, QueryOptions, SortField, Timestamp};
...
The conversion from SDK types to WIT types lives in the **guest module** of `omnia-wasi-docstore`. This follows the same pattern as `omnia-wasi-keyvalue/src/guest/cache.rs` -- the guest module wraps raw WIT bindings with a higher-level API that accepts SDK types.
...
|------|-------------|--------|
| SDK types (`omnia_guest::document_store`) | Hand-written | Platform-agnostic, used by domain crates |
| Guest WIT types (`omnia_wasi_docstore::types`) | `wit_bindgen::generate!` | wasm32 only |
| Host WIT types (`omnia_wasi_docstore::host::generated`) | `wasmtime::component::bindgen!` | Native only |
...
```rust
// crates/wasi-docstore/src/guest/store.rs

use anyhow::{Context, Result};
use omnia_guest::document_store as sdk;
...
```rust
// crates/wasi-docstore/src/guest/convert.rs
...
```rust
// crates/wasi-docstore/src/host.rs
...
```rust
// crates/wasi-docstore/src/host/resource.rs

use crate::host::generated::wasi::docstore::types::{ScalarValue, ComparisonOp};
...
```rust
// crates/wasi-docstore/src/host/types_impl.rs

impl HostFilterWithStore for WasiDocStore {
    fn compare<T>(
...
```rust
// crates/wasi-docstore/src/host/azure/filter.rs
...
```rust
// crates/wasi-docstore/src/host/mongodb/filter.rs
