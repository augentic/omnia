...
The `omnia.toml` file (or equivalent programmatic `omnia::Manifest`) declaring guests, mounts, routes, and links. See [Configuration](reference/configuration.md#deployment-manifest-omniatoml).

### Registry

The runtime's map from each opaque guest identity (`GuestId`) to a pre-instantiated guest, plus the route tables, mounts, and link allow-lists that hang off it. Per-request instantiation pulls from here.
...
Guest-to-guest calls carried through the host: a guest imports an interface named in a link allow-list, and the host routes the call to whichever guest exports it. Nesting is bounded by `MAX_DISPATCH_DEPTH`.
