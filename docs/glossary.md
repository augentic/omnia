...
The deployment's compiled-in `Acquirer` policy — how the loader turns a package name and location into component bytes. A composition-root value built by the `Wiring::acquirer` hook (the macro's declarative `plugins: { locations: [...] }` list lowers into it), never runtime-core machinery. One slot per location kind, filled by the built-in acquirers `PathAcquire` (named directory roots, read fresh on every load) and `RegistryAcquire` (exact package references, optionally cached in a `PluginStore`); loads route structurally by kind, and an empty slot refuses typed.
