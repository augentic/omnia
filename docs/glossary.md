...
One guest a deployment declares: a `[[guest]]` table, a `runtime!` `guests:` entry, or an `omnia::GuestEntry`. Its `name` is the guest's identity (`GuestId`) — what routes and link dispatch address, and what a `load("name")` answers with; omitted, the component file's stem names it (`echo.wasm` is `echo`) or the package reference without its version does (`acme:tool@1.2.3` is `acme:tool`). A manifest file's entry reads its component from a file (`source.path`) or fetches a package (`source.package`); the macro's embeds it (`path:`) or names a package (`package:`). Embedded bytes load at boot; every other entry is a [first-use guest](#first-use-guest).
...
Omnia's off-by-default `link` cargo feature: guest-to-guest dispatch via the `omnia-link` crate. Independent of the [`loader` feature](#loader-feature) — compiled-in guests may call each other; loaded guests may be host-only. Without it, an import outside the runtime's namespaces is unresolved and the deployment fails at boot.

### Loader feature

Omnia's off-by-default `loader` cargo feature: the [guest loader](#guest-loader) (`omnia-plugin`) and the registry client a `source.package` guest is fetched with. Independent of the [`link` feature](#link-feature). With it, `Deployment::assemble` links the loader host and hands the runtime its registry client for package sources; a deployment declaring `registries` or a package guest is refused at startup without it.

### Guest loader

The `omnia:plugins/loader` host capability: a guest names a location — `declared(name)`, one of the deployment's guests; `path(p)`, a component beneath one of its read-only [mounts](#mount); `registry(package, endpoint?)`, an exact package — and the host acquires the bytes inside the deployment's grant (its guest list, its mounts, its `registries` routing), verifies them against the digest, and admits them under the name the location derives (the declared name, the file stem, the package reference without its version), returning a typed handle carrying the identity and content digest. A `declared` name is one more first use of the runtime's guest seam, so a [first-use guest](#first-use-guest) loads there exactly as a route would load it; a name already active under the same bytes is attested; an undeclared name, a path beneath a writable mount, and a package nothing routes are refused. Component bytes never cross the interface, and the requester gains no lifecycle authority. Ships behind omnia's `loader` feature; linked at assembly for every deployment alike, reachable only from worlds that import it. The requester surface — the `Plugins` capability trait and the `Location`/`Plugin`/`Digest`/`Error` types — ships in `omnia-sdk`'s `plugins` module.

### First-use guest

A [guest entry](#guest-entry) whose component is not compiled into the runtime — every `[[guest]]` in a manifest file, and the macro's `package:` entries — and so loads the first time something names it: a route matching an event, the command drive (`command = true`), a link call, a host dispatch, or a `loader.load(declared(name))`. Nothing marks it; the source kind is the rule, and the retired `on_demand` key is refused. Its `source.path` is read at that first use, so `omnia compile` output loads from it only under a `digest` pin; its `source.package` is fetched by the `RegistryClient` the deployment's `registries` configuration routes it through — package override, namespace, then `default_registry`; a package routed nowhere is refused naming its namespace — or by the custom `RegistrySource` an embedder selected with `Deployment::registry_source`, optionally cached in a `ContentStore` + `ReleaseStore` backend, and admits raw wasm alone. A first-use guest is never the catch-all a trigger falls back to; it takes routes and the command mark like any other. The seam every first use goes through is `Runtime::guest`.
