...
The manifest itself — guests, routes, mounts, links — is covered in [Multi-Guest Deployments](multi-guest-deployments.md). A manifest can also be written inline in the macro instead of a TOML file; see [Inline manifest keys](../reference/runtime-macro.md#inline-manifest-keys-guests-mounts).
...
| --- | ----------------- |
| [`guests:`/`mounts:`](../reference/runtime-macro.md#inline-manifest-keys-guests-mounts) | You want the deployment compiled into the binary instead of a TOML file — including embedding the guest bytes themselves. |
