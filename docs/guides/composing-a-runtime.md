...
The [`guest-link-dynamic`](../../examples/guest-link/dynamic.rs) example is a complete host built this way. `run(builder)` builds the deployment as the generated `main` would: the builder is your binary's own input, so a pre-compiled guest it names — the binary's compiled-in `path:` overlaid with runtime declarations, say — loads without an `unsafe` call of your own (see the [security model](../security-model.md#deployment-inputs-are-trusted)). For still deeper control, implement the `omnia::Wiring` trait yourself, build a `Deployment` (the safe `build`, or `unsafe build_trusted` for pre-compiled artifacts), and call `omnia::run(deployment)`, or `deployment.assemble(backends)` to obtain an `omnia::Runtime<B>`. The [`crates/omnia` README](../../crates/omnia/README.md) lists the public API surface; [Architecture](../Architecture.md) explains how the pieces fit together.
