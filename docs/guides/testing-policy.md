...
- **The examples gate** (`examples/tests/examples.rs`) builds every example with the cargo commands the READMEs document (guests for `wasm32-wasip2`, hosts natively) and runs the run-to-completion hosts for exit status 0. It instantiates no guest of its own and asserts no behaviour — that lives in the e2e suites — so server examples are build-only.
