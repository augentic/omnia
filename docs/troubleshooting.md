...
It's probably running fine — servers are quiet by default. Run with `-v` and look for the `omnia ready` line. Without it, the only output is Cargo's `Running ...`.
...
One filter governs the host console and every guest: its bare level is a `-v`/`-q` flag in argv, else `RUST_LOG`'s, else the command-mode default `info`, and `RUST_LOG`'s targeted directives (`my_sdk=debug`, `tower=off`) apply on top whichever decided the level (see [Verbosity flags](reference/configuration.md#verbosity-flags)). Argv still reaches the guest verbatim, so a guest whose grammar does not flatten `omnia_sdk::api::command::Verbosity` rejects `-v` as an unknown flag *after* the host has applied the level — declare the flags. A bare run that logs too much has a process `RUST_LOG=debug` (or similar) set: `-q` steps the bare level down and keeps the variable's targets, as does any flag; a run that is still loud under `-q` has a *targeted* directive in `RUST_LOG` doing the talking, which no flag touches. Guest tracing is the guest's own subscriber reading the `RUST_LOG` its WASI environment carries — the decided filter — and nothing in the guest changes it at run time.
