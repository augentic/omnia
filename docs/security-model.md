...
- **The acquirer is host policy.** Acquisition (`Acquire`) is a value the composition root compiles in — the `Wiring::acquirer` hook, which the macro's `plugins: { acquire: ... }` key lowers into, invoked once after the deployment's backends connect. Core ships `MountAcquire`, which reads preopen-relative paths through the mount registry, fresh on every load, and refuses registry locations. No acquirer means every load refuses.
