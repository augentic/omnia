...
- **The acquirer is host policy.** Acquisition (the `Acquirer`, one slot per location kind) is a value the composition root compiles in — installed through `Plugins::install` from the `Wiring::extend` hook, which the macro's declarative `plugins: { locations: [...] }` list lowers into, invoked once after the deployment's backends connect and the runtime assembles. The built-in acquirers are `PathMounts` (named directory roots opened fail-fast at startup, read fresh on every load) and `RegistryClient` (exact package references from a compiled-in default registry endpoint, fresh-release-preferred when a `PluginStore` cache is attached). Every load routes structurally to its kind's slot, an empty slot refuses typed, and the reachable set is compiled into the binary — a guest cannot widen it. No acquirer means every load refuses.
