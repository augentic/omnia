...
- **The acquirer is host policy.** Acquisition (`Acquire`) is a value the composition root compiles in — the `Wiring::acquirer` hook, which the macro's `plugins: { acquire: ... }` key or declarative `locations:` list lowers into, invoked once after the deployment's backends connect. Core ships `MountAcquire` (preopen-relative reads through the mount registry, fresh on every load, registry locations refused), `PathAcquire` (named directory roots opened fail-fast at startup), and `RegistryAcquire` (exact package references from a compiled-in default registry endpoint, fresh-release-preferred when a `PluginStore` cache is attached). Whichever form, the reachable set is compiled into the binary — a guest cannot widen it. No acquirer means every load refuses.
