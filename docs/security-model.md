...
A runtime built with omnia's `loader` feature links the loader capability at assembly, letting a guest whose world imports it request that the host load another component at run time. The design keeps every trust decision host-side:

- **Request-only, byte-free.** The interface carries a location in and a typed handle out — component bytes structurally cannot cross it in either direction. A guest never supplies code; it names a registry package (and the registry to fetch it from, or none for the deployment's routing), a mount-relative path, or a guest the deployment already declared, and the host's compiled-in acquirer produces the bytes. Validation, compilation, and publication all happen host-side; the requester gains no lifecycle authority (no deregister, no mutation of mounts, hosts, or routes).
- **Acquisition is host policy.** The deployment's own data is the policy: its mounts are the roots path loads resolve against, and its `registries` configuration (the macro's `registries: include_str!("wasm-pkg.toml")`, a manifest's `[registries] path`) is the default routing of a package load that names no registry — `Deployment::assemble` installs both through `Plugins::install_declared`, or the custom sources an embedder selected with `Deployment::loader` before assembly. The built-in acquirers are `PathMounts` (the mounts, opened fail-fast at startup, read fresh on every load) and `RegistryClient` (exact package references, fetched from the registry the load names or routed by package override, namespace, then `default_registry` through the compiled-in wasm-pkg configuration — a package routed nowhere is refused before any registry is dialled — fresh-release-preferred when a store cache is attached by hand). Every load routes structurally to its kind's slot and an empty slot refuses typed. What a guest can reach is bounded by the deployment the operator chose: path loads by its mounts, registry loads by the endpoint the requesting guest's author wrote into it or the deployment's routing, declared loads by the guests it runs — a deployment with no mounts refuses every path load, and a load naming a registry reaches only that registry. The `registries` configuration is routing, not an allow-list: a load that names its registry fetches from it whatever the configuration says, so a deployment that must bound registry egress bounds it in the requesting guest — what endpoints that guest is willing to name — or does not build with `loader`.
- **Digest pins are operator-anchored.** A load may pin `sha256:<hex>`; the pin is verified against the acquired bytes *before* any wasmtime validation. Unpinned loads report the resolved digest on the returned handle (trust-on-first-use), so it can be committed as a pin. A re-load of an active package with a conflicting pin refuses. A declared guest is the deployment's own, so a load of it attests the registration and takes no pin.
