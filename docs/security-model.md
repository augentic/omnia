...
- **No process, clock, or environment escape.** Environment variables reach the guest only through `wasi:cli` argv/env the host chooses to forward — the process environment, with `RUST_LOG` set to the runtime's tracing filter — or `wasi:config`.
