...
- **Loaded plugins load as boot guests do.** The bytes an entry's source produces are loaded in whichever format they are, raw wasm or pre-compiled; the digest pin is the one check between acquisition and wasmtime, so a pre-compiled on-demand entry is one to pin.
...
- [ ] Take only raw `.wasm` from less-trusted sources, and run it with minimal hosts and read-only mounts
