...
- **The deployment's grant bounds every arm.** `Deployment::assemble` fixes the grant at install: the manifest's on-demand entries as the table a `declared` name may hit, the runtime's mounts as the roots a `path` may lie beneath, and the `registries` routing a `package` is fetched through. Nothing a caller passes widens it. A `declared` name outside the table is refused, and one already active (declared at boot, or admitted earlier) is attested without fetching anything; it takes no digest of its own — the entry carries the pin. A declared name is bound by its entry alone: a `path` or `package` that would register under it is refused before anything is read, so nothing a caller names can seat other bytes under the name for the declared load to attest. A `path` is read through the mount it lies beneath (`.` for a bare relative path, the mount's name as its prefix otherwise), with cap-std refusing any escape, and registers as its file stem. A `package` is fetched by the `RegistryClient` routed by the deployment's `registries` (the macro's `registries: include_str!("wasm-pkg.toml")`, a manifest's `[registries] path`) — package override, then namespace, then `default_registry`; the `endpoint` a load names serves only a namespace the deployment routes nowhere, and a package the deployment routes is refused rather than fetched elsewhere, so registry egress stays bounded by the operator's own manifest — or by the custom `RegistrySource` an embedder selected with `Deployment::registry_source` before assembly.
- **What a guest can write, it cannot run.** A component loads from a read-only mount alone: a `path` beneath a writable mount is refused before the file is read, so a guest with a writable mount cannot plant a component and have the host compile it. The loader also refuses at assembly a writable mount that shares or nests a read-only mount's directory, since a file written through the one view would load through the other; directories are told apart by identity, not path, so a bind mount or firmlink of a read-only mount's directory is refused as that directory. Two read-only mounts, or two writable ones, may nest freely. State a guest must write lives under its own mount; code lives under a read-only one.
- **Requester-named origins admit raw wasm alone.** A `path` or `package` the requester names is `wasm_only`: a pre-compiled artifact under it is refused however it hashes, so the worst such a load can name is a hostile guest inside the sandbox. A pre-compiled component is admitted only where the deployment itself declares it — a boot entry or an on-demand one the operator ships and pins.
...
- [ ] Pin every `[[guest]]` entry by `digest` wherever the artifact is known ahead of time, and pass a digest on every `path` or `registry` load whose bytes are known; treat an unpinned load's reported digest as the pin to commit
