...
| [`omnia-core`](crates/omnia-core)               | Live-runtime SDK — wasmtime engine, registry, dispatch, stores, and the tracing subscriber |
| [`omnia-plugin`](crates/omnia-plugin)           | `omnia:plugins/loader` capability — loader host, the deployment's grant (on-demand guest table, read-only mount roots, registry routing), registry acquisition, and plugin stores; selected by omnia's `loader` feature |
