...
- **Examples are gated by `examples/tests/examples.rs`**, which builds every example via the documented cargo commands (guests for `wasm32-wasip2`, hosts natively) and runs the run-to-completion ones for exit 0. It instantiates no guest itself and asserts no behaviour (that lives in the e2e suites); server examples are build-only.
