...
//!
//! Exports `example:link/echo` *and* re-imports it: each call consumes one hop
...
impl exports::example::link::echo::Guest for Relay {
    fn echo(target: String, message: String) -> String {
        match message.parse::<u32>() {
            Ok(hops) if hops > 0 => example::link::echo::echo(&target, &(hops - 1).to_string()),
