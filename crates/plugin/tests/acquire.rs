...
//! resolution, the store as fallback and byte cache, verify-before-persist,
//! poisoned entries, endpoint overrides, path locations, and acquirer
//! composition — all offline.

use std::path::{Path, PathBuf};

use omnia_plugin::{
    Acquire, AcquireContext, AcquireError, AcquireExt as _, DirStore, Location, PathAcquire,
    PluginStore as _, RegistryAcquire, sha256_digest,
};
...
const fn context() -> AcquireContext {
    AcquireContext { mounts: Vec::new() }
}
...
async fn store_miss_then_populates() {
    ...
    let entry = store.path().join("content").join(&digest);
...
async fn network_failure_falls_back_to_stored_record() {
    ...
    let warm =
        RegistryAcquire::new(UNROUTABLE_REGISTRY).with_config(config).cached_at(store.path());
    ...
    let offline = RegistryAcquire::new(UNROUTABLE_REGISTRY).cached_at(store.path());
...
async fn network_failure_without_record_refuses() {
    let store = TempDir::new().expect("store dir");
    let acquirer = RegistryAcquire::new(UNROUTABLE_REGISTRY).cached_at(store.path());
...
async fn poisoned_store_entry_discarded_and_refetched() {
    ...
    let entry = store.path().join("content").join(sha256_digest(b"honest bytes"));
    ...
    let healed = std::fs::read(&entry).expect("reading the store entry");
...
#[tokio::test]
async fn verify_before_persist_refuses_mismatched_bytes() {
    let root = TempDir::new().expect("store dir");
    let store = DirStore::new(root.path());
    let digest = sha256_digest(b"expected bytes");

    let error =
        store.put_content(&digest, b"tampered bytes").await.expect_err("mismatched bytes refused");
    assert!(format!("{error:#}").contains("refusing to persist"), "typed refusal: {error}");
    let entry = root.path().join("content").join(&digest);
...
async fn path_acquire_serves_its_own_locations() {
    ...
    // The context lends no mounts: PathAcquire resolves against its own
    // entries, never the deployment's.
