    ...
    S::StoreCtx: WebSocketView,
{
    ...
    PEER_MAP.set(Arc::<StdMutex<HashMap<SocketAddr, PeerInfo>>>::clone(&state))?;
...
/// Accepts a JSON string representing a `PublishMessage` with a peers field
/// (comma-separated list of peer addresses or "all") and a content field 
/// (the message to send)
