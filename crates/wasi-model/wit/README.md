...
p3 filesystem reads ride native component-model `stream`/`future` rather than `wasi:io` streams, so the p2-only `io.wit` that `filesystem/types` previously imported is no longer vendored.
