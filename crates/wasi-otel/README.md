...
Console output (events only, to stderr) follows the `RUST_LOG` the guest's WASI environment carries (the runtime sets it from its verbosity flags), defaulting to `error`; `flush` exports on demand.
