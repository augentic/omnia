...
//! resolve the guest by path through the static route table, instantiate it
//! fresh, hand it a `wasi:http` request, and convert the response back — but
//! skips the TCP socket and collects the response body eagerly so a test can
//! assert on it directly. The free helpers ([`handle`], [`get`], [`post`],
//! [`delete`], [`post_json`]) wrap a single-use harness; a scenario spanning
//! several requests must drive one harness so routing keeps the production
    ...
    StoreCtx<B>: WasiHttpView,
{
    ...
    /// the HTTP handler (mirroring the server's inert check).
    pub fn new(runtime: Runtime<B>) -> Result<Self> {
        // The runtime builds the router, so the harness cannot drift from the
        // production server's boot.
    ...
    /// Drive one request through the static route table and return its
    /// fully-collected response.
    ///
    /// # Errors
    ///
    /// Returns an error if the request path stays unmatched (`no route
    /// matched path`, the server's 404), the guest traps or returns an error,
    /// or the response cannot be converted and collected.
    pub async fn handle(&self, request: http::Request<Bytes>) -> Result<http::Response<Bytes>> {
        let path = request.uri().path().to_owned();
        let Some((guest_id, indices)) = self.routing.resolve(&path) else {
            return Err(anyhow!("no route matched path `{path}`"));
        };
        let guest =
