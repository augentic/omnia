...
Each `Host: Backend` pair links a WASI interface and binds it to a host backend. The macro always generates the `main` entry point; for a custom entry point, implement [`RuntimeHooks`] and call `omnia::run` (or build an `omnia::Runtime<B>` directly) instead.
...
| `RuntimeHooks<B>` | Per-deployment host linking and trigger-server startup (generated by `runtime!`).  |
...
- **Macros:** `runtime!`
- **Lifecycle:** [`run`](src/runtime.rs) (build → assemble → bootstrap → drive), `command::drive` — both server and command paths call `bootstrap` for epoch interruption, pool-metric sampling, and host-mediated link serving
