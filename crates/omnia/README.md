...
- **Registry pipeline:** `Manifest` (with `GuestEntry`, `Mount`, `RegistryConfig`, `SourceSpec`, route/transport types), `Source` (an entry resolved for loading, at boot for embedded bytes and at first use for a path or package), `DeploymentBuilder`, `Deployment`, `Registry`, `Guest`, `GuestId`, `RuntimeOptions`, `CompileOptions`
...
- **Telemetry:** `Telemetry` (the host's `tracing` subscriber: console logging with OTLP span and metric exporters beneath it), `telemetry::flush`, `telemetry::resource`, `LevelFilter`
- **CLI:** `Cli`, `Command`, `Parser` (`cli` feature)
- **Guest loader (`omnia:plugins/loader`, `loader` feature):** `WasiPlugins`, `Plugins`, `PluginLoader`, `Location`, the `RegistryClient` acquirer behind the `RegistrySource` seam (the trait itself lives in `omnia-core`, where the runtime fetches a `source.package` guest through it at first use), the `ContentStore`/`ReleaseStore` cache traits, and `Deployment::registry_source` for a custom registry; `RegistryConfig` is always available as manifest data, and a deployment declaring `registries` or a package guest requires the feature
...
- **`loader`:** Enables the guest loader (`omnia:plugins/loader`) and the registry client a `source.package` guest is fetched with. With it, `Deployment::assemble` links the loader host beside WASI — worlds that do not import it never see it — hands the runtime the registry client for its package guests, and installs the loader's grant: the deployment's guests as the names a `declared` load may name (loaded through the same first-use seam a route or a link call uses), the read-only mounts as the roots a `path` load reads through, and the deployment's `registries` as the routing every package is fetched by; a deployment declaring `registries` or a package guest requires it. A loaded guest is raw wasm, or `omnia compile` output where its source admits it (embedded bytes, a pinned path); compiling the former needs `jit`.
